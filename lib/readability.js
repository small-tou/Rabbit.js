...
// @include        *
// @exclude        http://*.qidian.com/*
// @run-at         document-start
// ==/UserScript==

/*
 * 基于Readability修改:
 *   1、完善中文下一页，修正下一页乱码的问题
...
var config = {
    ...
    footerlinks: true,   // 启用底部加载的所有链接列表
};
...
var dbg = config.debug ? function(s) {
...
var readability = {
            ...
            if(window.loadNextPageImmediately || config.loadNextPageImmediately){
                ...
                **/
                ...
                window.onscroll = function(){
                    ...
                    var nn = document.getElementById('readability-nextpagelinks').clientHeight;
                    if((aa-oo-nn) < 1000){
    ...
    **/
    ...
    getArticleTitle: function (page) {
        ...
        if(hOnes.length === 1){
            curTitle = origTitle = readability.getInnerText(hOnes[0]);
        }else{
            var title = page.getElementById('title');
            if(title){
                curTitle = origTitle =readability.getInnerText(title);
            }
        }
